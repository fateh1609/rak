...
// Placeholder images
const HERO_IMG = "https://i.ibb.co/N61WBzn8/Generated-Image-February-01-2026-1-19-AM.jpg";
const LOGO_URL = "https://i.ibb.co/kgQyG9mj/Generated-Image-February-01-2026-1-50-AM-removebg-preview.webp";
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // 2. Version & Cookie Logic
    const cachedVersion = localStorage.getItem('app_version');
    const hasConsented = localStorage.getItem('cookie_consent') === 'true';
    const isReturningUser = hasConsented && cachedVersion === APP_VERSION;
    ...
    // 3. Asset Preloading
    const preloadImage = (src: string) => new Promise((resolve) => {
      const img = new Image();
      img.src = src;
      img.onload = resolve;
      img.onerror = resolve; 
    ...
    const minDelay = isReturningUser ? 800 : 2500;
  ...
  const fetchProfile = async (userId: string) => {
    // Attempt to fetch extra profile data from 'profiles' table
    try {
        ...
        if (data) {
            ...
            // Fallback if profile row not created yet (or simple auth)
            // We use metadata or placeholders
            setProfile({
                id: userId,
  ...
  const handleAuthSuccess = () => {
      closeAuth();
      // Session update handled by onAuthStateChange listener
  };
  ...
  if (!isLoading && session) {
      return <ClientDashboard profile={profile} onLogout={handleLogout} />;
  }
  ...
  return (
    <div className="min-h-screen font-sans text-gray-800 bg-white relative selection:bg-gold-500 selection:text-white">
      {/* Preloader */}
      <Preloader logoUrl={LOGO_URL} isLoading={isLoading} />

      {/* Cookie Consent */}
      {showCookieBanner && !isLoading && (
      ...
      <nav className="absolute w-full z-40 top-0 left-0 border-b border-white/10 backdrop-blur-md bg-deepblue-900/30 transition-all duration-300">
        <div className="container mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          ...
          <div className="hidden lg:flex gap-8 text-white/90 font-light tracking-wide text-sm items-center">
            ...
            <Button variant="outline" onClick={openAuth} className="!px-6 !py-2 !text-xs uppercase tracking-widest hover:!bg-gold-500 hover:!border-gold-500 hover:!text-white">
              Client Portal
            </Button>
          </div>
      ...
      <section className="relative min-h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden">
          <img 
            ...
            className={`w-full h-full object-cover animate-ken-burns origin-center transition-opacity duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
            fetchPriority="high"
          />
          {/* Gradient Overlays */}
        ...
        <div className="container mx-auto px-4 md:px-6 z-10 relative mt-24 md:mt-10">
          ...
          <div className="md:hidden mt-12 grid grid-cols-2 gap-3 animate-fade-in-up">
              <div className="bg-deepblue-900/60 backdrop-blur-xl border border-white/10 p-4 rounded-xl text-center">
                  <div className="text-2xl font-serif text-white mb-1">406 ACRES</div>
                  <div className="text-[10px] text-gold-400 uppercase tracking-widest font-semibold">Masterplan</div>
              </div>
              <div className="bg-deepblue-900/60 backdrop-blur-xl border border-white/10 p-4 rounded-xl text-center">
                  <div className="text-2xl font-serif text-white mb-1">AED 101</div>
                  <div className="text-[10px] text-gold-400 uppercase tracking-widest font-semibold">Per Sq.Ft</div>
              </div>
              <div className="bg-deepblue-900/60 backdrop-blur-xl border border-white/10 p-4 rounded-xl text-center">
                  <div className="text-2xl font-serif text-white mb-1">5 YEARS</div>
                  <div className="text-[10px] text-gold-400 uppercase tracking-widest font-semibold">Payment Plan</div>
              </div>
              <div className="bg-deepblue-900/60 backdrop-blur-xl border border-white/10 p-4 rounded-xl text-center">
                  <div className="text-2xl font-serif text-white mb-1">0%</div>
                  <div className="text-[10px] text-gold-400 uppercase tracking-widest font-semibold">Interest</div>
              </div>
          </div>
        </div>
      ...
      <AuthModal isOpen={isAuthModalOpen} onClose={closeAuth} onSuccess={handleAuthSuccess} />
