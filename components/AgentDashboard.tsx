...
import { DashboardHome } from './agent/Home';
import { NetworkView } from './agent/Network';
import { SalesView } from './agent/Sales';
import { EarningsView } from './agent/Earnings';
import { LeaderboardView } from './agent/Leaderboard';
import { RecruitAgent } from './agent/Recruit';
import { MarketingView } from './agent/Marketing';
import { ProfileView } from './agent/Profile';
import { SupportView } from './agent/Support';
...
export const AgentDashboard: React.FC<DashboardProps> = ({ profile, onLogout, onNavigate }) => {
  ...
  const renderContent = () => {
      ...
      if (view !== 'DASHBOARD' && settings.agent[view] === 'DISABLED') {
          return <MaintenanceOverlay onBack={() => navigateTo('DASHBOARD')} title={`${view} Unavailable`} />;
      }
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[250px] bg-deepblue-900 text-white transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 shadow-2xl flex flex-col border-r border-white/5`}>
         ...
         <nav className="flex-1 p-4 space-y-1 overflow-y-auto no-scrollbar">
             {navItems.map((item, index) => {
                 ...
                 if (!item.id || settings.agent[item.id] === 'HIDDEN') return null;
                 ...
                 return (
                     <NavItem 
                        ...
                        icon={item.icon} 
                        label={item.label} 
                        active={view === item.id || (view === 'RECRUIT' && item.id === 'NETWORK')} 
                        onClick={() => navigateTo(item.id as AgentView)} 
                     />
                 );
             })}
         </nav>
          ...
          <header className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 shrink-0 sticky top-0 z-30 shadow-sm">
              <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-xl font-bold text-deepblue-900 hidden sm:block">
                      {view === 'DASHBOARD' ? `Welcome, ${profile?.full_name?.split(' ')[0]}!` : view.charAt(0) + view.slice(1).toLowerCase().replace('_', ' ')}
                  </h2>
